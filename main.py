...
def get_shared_style(fg_color):
    ...
    <style>
            body {{
                ...
                margin: 20px;
                /* Mobile: Default larger font base */
                font-size: 16px;
            }}
            h1 {{
                color: #333;
            ...
            table {{
                ...
                font-size: 1em;
                white-space: normal;
                table-layout: auto; /* Let browser optimize widths */
            }}
            th, td {{
                border: 1px solid #ccc;
                /* Mobile: Tighter horizontal padding to save space */
                padding: 8px 4px;       
                text-align: left;
            }}
            th {{
            ...
            /* Sort Indicators */
            .sortable th {{ cursor: pointer; color: #007bff; text-decoration: underline; }}
            .sortable th.asc::after {{ content: " ▲"; color: #333; font-size: 0.8em; }}
            .sortable th.desc::after {{ content: " ▼"; color: #333; font-size: 0.8em; }}

            /* Mobile Specific Overrides (Screen width < 600px) */
            @media (max-width: 37.5em) {{
                ...
                h1 {{ font-size: 1.5em; }}
                th, td {{
                    padding: 8px 2px; /* Even tighter on very small screens */
                    font-size: 0.95em;
                }}
                /* Force Industry column to wrap to save width */
                td:nth-child(4) {{
            ...
            @media (min-width: 64em) {{   
                ...
                th, td {{ padding: 8px 10px; }} /* Relaxed padding on desktop */
            }}
    </style>
    <script>
    (function () {
            ...
            document.querySelectorAll("table.sortable").forEach(table => {{
                ...
                headers.forEach((header, i) => {{
                    header.addEventListener("click", () => {{
                        ...
                        // FIX: Select ALL rows in tbody (do not skip first child)
...
def sector_counts_to_html(title, sector_counts):
    if not sector_counts: return "<p>No sector data.</p>"
...
def write_index_html(daily, weekly, daily_sec, weekly_sec, fg_data, report_date_str):
    ...
    <div class="row">
        <div class="column">
            <h3>Daily Bottoms</h3>
            {signals_to_html_table(daily["Bottoms"])}
            {sector_counts_to_html("Daily Bottoms by Sector", daily_sec["Bottoms"])}
        </div>
        <div class="column">
            <h3>Daily Tops</h3>
            {signals_to_html_table(daily["Tops"])}
            {sector_counts_to_html("Daily Tops by Sector", daily_sec["Tops"])}
        </div>
    </div>
    <div class="row">
        <div class="column">
            <h3>Weekly Bottoms</h3>
            {signals_to_html_table(weekly["Bottoms"])}
            {sector_counts_to_html("Weekly Bottoms by Sector", weekly_sec["Bottoms"])}
        </div>
        <div class="column">
            <h3>Weekly Tops</h3>
            {signals_to_html_table(weekly["Tops"])}
            {sector_counts_to_html("Weekly Tops by Sector", weekly_sec["Tops"])}
        </div>
    </div>
...
def write_wyckoff_html(wyckoff_res, report_date_str):
    ...
    try:
        with open("docs/wyckoff.html", "w", encoding="utf-8") as f:
            f.write(html)
        print(f"✅ Successfully wrote docs/wyckoff.html")
    except Exception as e:
        print(f"❌ Failed to write Wyckoff file: {e}")
...
def main():
    ...
    # Pass sector counts (daily_sec, weekly_sec) to HTML writer
    write_index_html(daily_res, weekly_res, daily_sec, weekly_sec, fg_data, subtitle)
