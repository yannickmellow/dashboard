...
# ==========================================
# 5. HTML GENERATION (ORIGINAL STYLE)
# ==========================================
...
def get_shared_style():
    ...
    <style>
        ...
        .sector-grid { display: flex; flex-wrap: wrap; margin-bottom: 30px; width: 100%; }
        .sector-cell { border: 1px solid #ccc; padding: 12px 14px; text-align: center; font-weight: bold; flex: 0 0 33.33%; box-sizing: border-box; }
        
        @media (min-width: 64em) {
            .row { flex-direction: row; }
            .column { margin: 0 10px; }
            .summary-table { width: 60%; }
            .sector-cell { flex: 0 0 16.66%; }
        }
...
def write_html_pages(daily, weekly, daily_sec, weekly_sec, fg_data, wyckoff_res, report_date):
    ...
    # --- PAGE 1: DEMARK INDEX ---
    html_dm = f"""<html><head><meta charset="UTF-8"><title>US DM Dashboard</title>{get_shared_style()}</head><body>
    <div>
    ...
    <div class="row">
        <div class="column">
            <h3>Daily Bottoms</h3>{signals_to_html_table(daily["Bottoms"])}
        </div>
        <div class="column">
            <h3>Daily Tops</h3>{signals_to_html_table(daily["Tops"])}
        </div>
    </div>
    <div class="row">
        <div class="column">
            <h3>Weekly Bottoms</h3>{signals_to_html_table(weekly["Bottoms"])}
        </div>
        <div class="column">
            <h3>Weekly Tops</h3>{signals_to_html_table(weekly["Tops"])}
        </div>
    </div>
    ...
    # --- PAGE 2: WYCKOFF ---
    print(f"✍️ Writing docs/wyckoff.html with {len(wyckoff_res)} candidates...")
    
    # Build Wyckoff Rows
    w_rows = ""
    for t, price, sec, ind, pct in wyckoff_res:
        color = "green" if pct > 0 else "red"
        # Strict 2 decimal formatting for price and percentage
        w_rows += f"""<tr>
    ...
    if not w_rows: w_rows = "<tr><td colspan='6'>No Wyckoff candidates found today.</td></tr>"

    html_wyckoff = f"""<html><head><meta charset="UTF-8"><title>Wyckoff Screener</title>{get_shared_style()}</head><body>
    <div>
...
def main():
    ...
    scan_timeframe(sec_map, sec_ind, "Sector", "1d") # Just to run it

    # 3. Wyckoff Scan
    wyckoff_res = scan_wyckoff_signals(maps, industries)
    print(f"💪 Wyckoff Candidates: {len(wyckoff_res)}")
    ...
    write_html_pages(daily_res, weekly_res, daily_sec, weekly_sec, fg_data, wyckoff_res, report_date)
    
    print("\n✅ Done. Check docs/wyckoff.html")
